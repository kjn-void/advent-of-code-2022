...
func TestDay18_1(t *testing.T) {
	...
	numFaces := CountFreeFaces(world.cubes)
	if numFaces != 10 {
...
func TestDay18_3(t *testing.T) {
	...
	numFaces := world.CountExteriorFreeFaces(CountFreeFaces(world.cubes))
	if numFaces != 6 {
...
func BenchmarkDay18_part2(b *testing.B) {
	world := parseCubeWorld(inputAsString(18))
	for n := 0; n < b.N; n++ {
		world.CountExteriorFreeFaces(CountFreeFaces(world.cubes))
	}
}
